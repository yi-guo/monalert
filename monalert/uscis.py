...
from typing import Dict, Final, List, NoReturn, Optional, TypedDict, Union
...
class USCIS(models.Monalert):
    ...
    def _get_notification(self) -> Dict[str, Union[str, int]]:
        ...
        title: str = \
            f"USCIS {receipt_num_masked}: {self.__curr_status['status']}"
        message: str = self.__curr_status["description"].replace(
            self.__receipt_num,
            receipt_num_masked,
        )
    ...
    def __get_prev_status(self) -> Optional[Status]:
        cursor: pymongo.cursor.Cursor = self.__mongo_collection.find({
            "receipt_num":
            self.__receipt_num,
        }).sort("utc_time", direction=pymongo.DESCENDING).limit(1)
    ...
    def __save_curr_status(self, curr_status: Status) -> NoReturn:
